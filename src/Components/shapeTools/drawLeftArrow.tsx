...
export default function DrawLeftArrow({
  xAxis,
  yAxis,
  parentHeight,
  resetLine,
  drawLeftArrowOnCanvas,
}) {
  const { paperDimensions, primaryColor, brushWidth } = useContext(MainContext);

  const LeftLeftArrowRef = useRef(null);
  ...
  useEffect(() => {
    ...
    if (LeftLeftArrowRef.current) {
      LeftLeftArrowRef.current.addEventListener("mousedown", (e) => {
    ...
    return () => {
      if (LeftLeftArrowRef.current) {
        LeftLeftArrowRef.current.removeEventListener("mousedown", (e) =>
          setMoveSelectBox(() => {
            return { status: true, x: e.clientX, y: e.clientY };
          })
        );
  ...
  useEffect(() => {
    function removeSelectBox(e) {
      if (!hoverActive && !resize.status && !preventDeletion) {
        drawLeftArrowOnCanvas(pointArray);
        resetLine();
