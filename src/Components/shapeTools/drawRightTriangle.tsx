...
export default function DrawRightTriangle({
  xAxis,
  yAxis,
  parentHeight,
  resetLine,
  drawRightTriangleOnCanvas,
}) {
  ...
  useEffect(() => {
    ...
    if (RightTriangleRef.current) {
      RightTriangleRef.current.addEventListener("mousedown", (e) => {
    ...
    return () => {
      if (RightTriangleRef.current) {
        RightTriangleRef.current.removeEventListener("mousedown", (e) =>
          setMoveSelectBox(() => {
            return { status: true, x: e.clientX, y: e.clientY };
          })
        );
  ...
  useEffect(() => {
    function removeSelectBox(e) {
      if (!hoverActive && !resize.status && !preventDeletion) {
        drawRightTriangleOnCanvas(startX, startY, endX, endY);
        resetLine();
