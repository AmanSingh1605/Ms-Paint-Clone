...
export default function DrawDiamond({
  xAxis,
  yAxis,
  parentHeight,
  resetLine,
  ...
  useEffect(() => {
    ...
    if (DiamondRef.current) {
      DiamondRef.current.addEventListener("mousedown", (e) => {
    ...
    return () => {
      if (DiamondRef.current) {
        DiamondRef.current.removeEventListener("mousedown", (e) =>
          setMoveSelectBox(() => {
            return { status: true, x: e.clientX, y: e.clientY };
          })
        );
  ...
  useEffect(() => {
    function removeSelectBox(e) {
      if (!hoverActive && !resize.status && !preventDeletion) {
        drawDiamondOnCanvas(startX, startY, endX, endY);
        resetLine();
  ...
  return (
    ...
    >
      <svg height={paperDimensions.height} width={paperDimensions.width}>
        <polygon
          points={`${startX}, ${(startY + endY) / 2}, ${(endX + startX) / 2}, ${startY}, ${endX},${(startY + endY) / 2}, ${(startX + endX) / 2}, ${endY}`}
