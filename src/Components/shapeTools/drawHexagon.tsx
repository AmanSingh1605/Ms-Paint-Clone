...
export default function DrawHexagon({
  ...
  useEffect(() => {
    ...
    if (HexagonRef.current) {
      HexagonRef.current.addEventListener("mousedown", (e) => {
    ...
    return () => {
      if (HexagonRef.current) {
        HexagonRef.current.removeEventListener("mousedown", (e) =>
          setMoveSelectBox(() => {
            return { status: true, x: e.clientX, y: e.clientY };
          })
        );
  ...
  useEffect(() => {
    function removeSelectBox(e) {
      if (!hoverActive && !resize.status && !preventDeletion) {
        drawHexagonOnCanvas(startX, startY, endX, endY);
        resetLine();
  ...
  return (
    ...
    >
      <svg height={paperDimensions.height} width={paperDimensions.width}>
        <polygon
          points={`${(startX + endX) / 2}, ${startY}, ${endX}, ${startY + (endY - startY) * 0.2},${endX}, ${startY + (endY - startY) * 0.8},${(endX + startX) / 2},${endY},${startX}, ${startY + (endY - startY) * 0.8},${startX}, ${startY + (endY - startY) * 0.2}`}
