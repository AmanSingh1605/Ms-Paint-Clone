...
export default function DrawSixPointStar({
  xAxis,
  yAxis,
  parentHeight,
  resetLine,
  drawSixPointStarOnCanvas,
}) {
  const { paperDimensions, primaryColor, brushWidth } = useContext(MainContext);

  const LeftFivePointStarRef = useRef(null);
  ...
  const polygonPoints = pointArray.join(" ");
  ...
  useEffect(() => {
    ...
    if (LeftFivePointStarRef.current) {
      LeftFivePointStarRef.current.addEventListener("mousedown", (e) => {
    ...
    return () => {
      if (LeftFivePointStarRef.current) {
        LeftFivePointStarRef.current.removeEventListener("mousedown", (e) =>
          setMoveSelectBox(() => {
            return { status: true, x: e.clientX, y: e.clientY };
          })
        );
  ...
  useEffect(() => {
    function removeSelectBox(e) {
      if (!hoverActive && !resize.status && !preventDeletion) {
        drawSixPointStarOnCanvas(pointArray);
        resetLine();
