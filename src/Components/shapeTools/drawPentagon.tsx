...
export default function DrawPentagon({
  xAxis,
  yAxis,
  parentHeight,
  resetLine,
  drawPentagonOnCanvas,
}) {
  ...
  const center_x = (startX + endX) / 2;
  const center_y = (startY + endY) / 2;
  const side_length = Math.sqrt((endX - startX) ** 2 + (endY - startY) ** 2);

  const vertices = Array.from({ length: 5 }, (_, i) => {
    const angle = (2 * i * Math.PI) / 5;
    const vertex_x = center_x + (side_length / 2) * Math.cos(angle);
    const vertex_y = center_y + (side_length / 2) * Math.sin(angle);
    return `${vertex_x},${vertex_y}`;
  });

  const polygonPoints = vertices.join(" ");
  ...
  useEffect(() => {
    ...
    if (PentagonRef.current) {
      PentagonRef.current.addEventListener("mousedown", (e) => {
    ...
    return () => {
      if (PentagonRef.current) {
        PentagonRef.current.removeEventListener("mousedown", (e) =>
          setMoveSelectBox(() => {
            return { status: true, x: e.clientX, y: e.clientY };
          })
        );
  ...
  useEffect(() => {
    function removeSelectBox(e) {
      if (!hoverActive && !resize.status && !preventDeletion) {
        drawPentagonOnCanvas(startX, startY, endX, endY);
        resetLine();
  ...
  return (
    ...
    >
      <svg height={paperDimensions.height} width={paperDimensions.width}>
        <polygon
          points={`${(startX + endX) / 2}, ${startY}, ${startX}, ${startY + (endY - startY) * 0.33},${startX + (endX - startX) * 0.2},${endY},${startX + (endX - startX) * 0.8}, ${endY}, ${endX}, ${startY + (endY - startY) * 0.33}`}
          style={{ stroke: primaryColor, strokeWidth: brushWidth }}
          strokeLinejoin="round"
          className=" fill-none cursor-move"
