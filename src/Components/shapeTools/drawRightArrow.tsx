...
export default function DrawRightArrow({
  xAxis,
  yAxis,
  parentHeight,
  resetLine,
  drawRightArrowOnCanvas,
}) {
  const { paperDimensions, primaryColor, brushWidth } = useContext(MainContext);

  const LeftRightArrowRef = useRef(null);
  ...
  useEffect(() => {
    ...
    if (LeftRightArrowRef.current) {
      LeftRightArrowRef.current.addEventListener("mousedown", (e) => {
    ...
    return () => {
      if (LeftRightArrowRef.current) {
        LeftRightArrowRef.current.removeEventListener("mousedown", (e) =>
          setMoveSelectBox(() => {
            return { status: true, x: e.clientX, y: e.clientY };
          })
        );
  ...
  useEffect(() => {
    function removeSelectBox(e) {
      if (!hoverActive && !resize.status && !preventDeletion) {
        drawRightArrowOnCanvas(pointArray);
        resetLine();
