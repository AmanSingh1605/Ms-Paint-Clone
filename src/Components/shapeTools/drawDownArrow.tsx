...
export default function DrawDownArrow({
  xAxis,
  yAxis,
  parentHeight,
  resetLine,
  drawDownArrowOnCanvas,
}) {
  const { paperDimensions, primaryColor, brushWidth } = useContext(MainContext);

  const LeftDownArrowRef = useRef(null);
  ...
  useEffect(() => {
    ...
    if (LeftDownArrowRef.current) {
      LeftDownArrowRef.current.addEventListener("mousedown", (e) => {
    ...
    return () => {
      if (LeftDownArrowRef.current) {
        LeftDownArrowRef.current.removeEventListener("mousedown", (e) =>
          setMoveSelectBox(() => {
            return { status: true, x: e.clientX, y: e.clientY };
          })
        );
  ...
  useEffect(() => {
    function removeSelectBox(e) {
      if (!hoverActive && !resize.status && !preventDeletion) {
        drawDownArrowOnCanvas(pointArray);
        resetLine();
