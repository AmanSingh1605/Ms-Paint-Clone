...
export default function DrawUpArrow({
  xAxis,
  yAxis,
  parentHeight,
  resetLine,
  ...
  const LeftUpArrowRef = useRef(null);
  ...
  useEffect(() => {
    ...
    if (LeftUpArrowRef.current) {
      LeftUpArrowRef.current.addEventListener("mousedown", (e) => {
    ...
    return () => {
      if (LeftUpArrowRef.current) {
        LeftUpArrowRef.current.removeEventListener("mousedown", (e) =>
          setMoveSelectBox(() => {
            return { status: true, x: e.clientX, y: e.clientY };
          })
        );
  ...
  useEffect(() => {
    function removeSelectBox(e) {
      if (!hoverActive && !resize.status && !preventDeletion) {
        drawUpArrowOnCanvas(pointArray);
        resetLine();
