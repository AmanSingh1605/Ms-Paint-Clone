...
export default function DrawTriangle({
  ...
  useEffect(() => {
    ...
    if (triangleRef.current) {
      triangleRef.current.addEventListener("mousedown", (e) => {
    ...
    return () => {
      if (triangleRef.current) {
        triangleRef.current.removeEventListener("mousedown", (e) =>
          setMoveSelectBox(() => {
            return { status: true, x: e.clientX, y: e.clientY };
          })
        );
  ...
  useEffect(() => {
    function removeSelectBox(e) {
      if (!hoverActive && !resize.status && !preventDeletion) {
        drawTriangleOnCanvas(startX, startY, endX, endY);
        resetLine();
  ...
  return (
    ...
    >
      <svg height={paperDimensions.height} width={paperDimensions.width}>
        <polygon
          points={`${startX}, ${endY}, ${(endX + startX) / 2}, ${startY}, ${endX},${endY}`}
